...
# Operations:
#
# test - Run tests, using compiled-in configuration, and compare to existing reference results.
#
# testcfg - Run tests, reading configuration from ./oafcfg, and compare to existing reference results.
#
# testaafs - Run tests, reading configuration from /opt/aafs/oafcfg, and compare to existing reference results.
#
...
runut () {
    ...
    if [ -z "$CFGDIR" ]; then
        ...
        java -Doafcfg="$CFGDIR" -Dtesttime=$testtime -cp opensha-oaf/build/libs/oefjava.jar:opensha-oaf/lib/ProductClient.jar $JCLASS "$@" > "$TMPFILE2" 2>&1
    fi
...
rundbut () {
    ...
    if [ -z "$CFGDIR" ]; then
        ...
        java -Doafcfg="$CFGDIR" -Dtesttime=$testtime -Dtestconopt=$CONOPT -cp opensha-oaf/build/libs/oefjava.jar:opensha-oaf/lib/ProductClient.jar $JCLASS "$@" > "$TMPFILE2" 2>&1
    fi

    # Remove the lines that are generated by MongoDB

    cat "$TMPFILE2" | sed '/mongodb\.diagnostics/d' | sed '/mongodb\.internal/d' | sed '/^INFO: /d' | cat "$TMPFILE1" - > "$CURFILE"
...
case "$2" in
    ...
    # Run tests, reading configuration from ./oafcfg
    ...
    # Run tests, reading configuration from /opt/aafs/oafcfg

    testaafs)

        # Configuration directory

        CFGDIR="/opt/aafs/oafcfg"
    ...
    *)
        echo "Usage: utoaf.sh unit {test|testcfg|testaafs|save|cleancur|cleanref|cur|ref|diff}"
        exit 1
        ;;
esac
