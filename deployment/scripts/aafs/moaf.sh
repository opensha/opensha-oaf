...
#
#     <configured_primary> selects which server is primary. It can have two values:
#
...
#
#     Notice that the relay mode does not change instantaneously.  You can use
#     the show_relay_status command to see when the change has taken effect.
#
...
#
#     <srvnum> is the server from which you are obtaining status information request.
#     It can have one of four values:
#
#       0 - Retrieve status from the local server (the server on which you
#           are running the command).
#
#       1 - Retrieve status from server #1 in a dual-server configuration,
#           or from the only server in a single-server configuration.
#
#       2 - Retrieve status from server #2 in a dual-server configuration.
#
#       9 - Retrieve status from both servers in a dual-server configuration.
#
