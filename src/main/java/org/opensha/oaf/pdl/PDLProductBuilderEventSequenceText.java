...
/**
 * Code to construct the PDL product of type "event-sequence-text".
 * Author: Michael Barall 09/14/2022.
 */
public class PDLProductBuilderEventSequenceText {
	 ...
	 * Note: productFiles must include at least the summary.html file, however this is not enforced here.
	 * The summary.html filename is available as PropertiesEventSequence.EVSTEXT_FILENAME_SUMMARY.
	 * @return
	 *     The product that was created
	 */
	public static Product createProduct (String eventID, String eventNetwork, String eventCode, Map<String, String> extra_properties,
...
//		attachContentToProduct(product);    // file- or byte-based content
		...
		if (eventID == null || eventID.isEmpty()) {
			throw new IllegalArgumentException ("PDLProductBuilderEventSequenceText: Event ID is not specified");
		}
		if (eventNetwork == null || eventNetwork.isEmpty()) {
			throw new IllegalArgumentException ("PDLProductBuilderEventSequenceText: Event network identifier is not specified");
		}
		if (eventCode == null || eventCode.isEmpty()) {
			throw new IllegalArgumentException ("PDLProductBuilderEventSequenceText: Event network code is not specified");
		}

		// Announce it

		System.out.println ("Constructing PDL event-sequence-text product: event ID = " + eventID + ", network = " + eventNetwork + ", code = " + eventCode);

		// Our PDL source, type, and code

		//String source = "us";
		//String type = "event-sequence-text";

		ServerConfig server_config = new ServerConfig();
		String source = server_config.get_pdl_oaf_source();
		String type = ComcatProduct.PRODTYPE_EVENT_SEQUENCE_TEXT;
		...
		attachPropertiesToProduct (product, eventNetwork, eventCode, extra_properties, isReviewed);
	 ...
	 *     The product that was created
	 */
	public static Product createDeletionProduct (String eventID, String eventNetwork, String eventCode, Map<String, String> extra_properties, boolean isReviewed, long modifiedTime) throws Exception {
		Product product;
		ProductId productId;
...
//		attachContentToProduct(product);    // file- or byte-based content
		...
		System.out.println ("Constructing PDL deletion event-sequence-text product: event ID = " + eventID + ", network = " + eventNetwork + ", code = " + eventCode);
...
//		// use String.getBytes() to generate ByteContent for a product.
		...
		if (jsonText == null || jsonText.isEmpty()) {
			throw new IllegalArgumentException ("PDLProductBuilderEventSequenceText: JSON text for product content is not specified");
		}
...
//				new File("./test-content")));
	 ...
	 * @param isReviewed = True if this product has been reviewed.
	 */
	public static void attachPropertiesToProduct (Product product, String eventNetwork, String eventCode, Map<String, String> extra_properties, boolean isReviewed) {
