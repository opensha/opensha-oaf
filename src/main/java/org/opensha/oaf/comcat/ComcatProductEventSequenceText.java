...
/**
 * A Comcat event sequence text product, as retrieved from Comcat.
 * Author: Michael Barall 01/08/2022.
 *
 * This class is preliminary.
 */
public class ComcatProductEventSequenceText extends ComcatProduct {
	...
	private static final String default_product_type = PRODTYPE_EVENT_SEQUENCE_TEXT;
	...
	protected boolean read_from_gj (JSONObject gj_product, boolean delete_ok) {
		...
		// For event sequence text, there must be a summary.html file for a non-delete product

		if (!( isDelete )) {

			if (!( contains_file (PropertiesEventSequence.EVSTEXT_FILENAME_SUMMARY) )) {
				return false;
			}
	...
	public static ComcatProductEventSequenceText make_from_gj (JSONObject gj_product) {
		return make_from_gj (gj_product, false);
	}

	public static ComcatProductEventSequenceText make_from_gj (JSONObject gj_product, boolean delete_ok) {
		ComcatProductEventSequenceText the_product = new ComcatProductEventSequenceText();
	...
	public static ComcatProductEventSequenceText make_preferred_from_gj (JSONObject event) {
		return make_preferred_from_gj (event, false);
	}

	public static ComcatProductEventSequenceText make_preferred_from_gj (JSONObject event, boolean delete_ok) {
		int index = 0;
	...
	public static List<ComcatProductEventSequenceText> make_list_from_gj (JSONObject event) {
		return make_list_from_gj (event, false);
	}

	public static List<ComcatProductEventSequenceText> make_list_from_gj (JSONObject event, boolean delete_ok) {
		ArrayList<ComcatProductEventSequenceText> the_product_list = new ArrayList<ComcatProductEventSequenceText>();
		...
		if (gj_product_array != null) {
			...
			for (int k = 0; k < gj_product_array.size(); ++k) {
				...
				if (gj_product != null) {
					ComcatProductEventSequenceText the_product = make_from_gj (gj_product, delete_ok);
	...
	public static void main(String[] args) {
		...
		if (args.length < 1) {
			System.err.println ("ComcatProductEventSequenceText : Missing subcommand");
			return;
		}
		...
		if (args[0].equalsIgnoreCase ("test1")) {
			...
			if (args.length != 4 && args.length != 5) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test1' subcommand");
				return;
			}
			...
			try {
				...
				ComcatProductEventSequenceText preferred_product = make_preferred_from_gj (accessor.get_last_geojson());

				//  ComcatProductEventSequenceText preferred_product = make_preferred_from_gj (product_type, accessor.get_last_geojson());
		...
		if (args[0].equalsIgnoreCase ("test2")) {
			...
			if (args.length != 6 && args.length != 7) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test2' subcommand");
				return;
			}
			...
			try {
				...
				List<ComcatProductEventSequenceText> product_list = make_list_from_gj (accessor.get_last_geojson(), delete_ok);

				//  List<ComcatProductEventSequenceText> product_list = make_list_from_gj (product_type, accessor.get_last_geojson(), delete_ok);
		...
		if (args[0].equalsIgnoreCase ("test3")) {
			...
			if (args.length != 5 && args.length != 6) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test3' subcommand");
				return;
			}
		...
		if (args[0].equalsIgnoreCase ("test4")) {
			...
			if (args.length != 4 && args.length != 5) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test4' subcommand");
				return;
			}
		...
		if (args[0].equalsIgnoreCase ("test6")) {
			...
			if (args.length != 6 && args.length != 7) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test6' subcommand");
				return;
			}
		...
		if (args[0].equalsIgnoreCase ("test7")) {
			...
			if (args.length != 5 && args.length != 6) {
				System.err.println ("ComcatProductEventSequenceText : Invalid 'test7' subcommand");
				return;
			}
		...
		System.err.println ("ComcatProductEventSequenceText : Unrecognized subcommand : " + args[0]);
		return;
