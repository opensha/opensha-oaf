...
public class RJGUIController extends RJGUIListener {
	...
	private boolean filter_state (int ctrl_modstate, int filtopt) throws GUIEDTException {
		...
		if (!( ctrl_modstate >= MODSTATE_MIN && ctrl_modstate <= MODSTATE_MAX )) {
			throw new IllegalArgumentException ("RJGUIController.filter_state - Invalid control state: " + get_modstate_as_string(ctrl_modstate));
		}
		...
		default:
			...
			throw new IllegalArgumentException ("RJGUIController.filter_state - Invalid filter option: " + get_filtopt_as_string(filtopt));
		}
	...
	private void advance_state (int new_modstate) throws GUIEDTException {
		...
		if (!( new_modstate >= MODSTATE_MIN && new_modstate <= MODSTATE_MAX )) {
			throw new IllegalArgumentException ("RJGUIController.advance_state - Invalid control state: " + get_modstate_as_string(new_modstate));
		}
