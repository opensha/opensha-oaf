...
public abstract class RJOldGUIListener extends RJOldGUIComponent implements ParameterChangeListener {
	...
	protected class ChangeBlock implements AutoCloseable {
		...
		ChangeBlock () {
			if (!( SwingUtilities.isEventDispatchThread() )) {
				throw new IllegalStateException("RJOldGUIListener.ChangeBlock called while not on the event dispatch thread!");
			}
			synchronized (RJOldGUIListener.this) {
				++change_block_count;
			}
		}
	...
	protected class ChangeDepth implements AutoCloseable {
		...
		ChangeDepth () {
			if (!( SwingUtilities.isEventDispatchThread() )) {
				throw new IllegalStateException("RJOldGUIListener.ChangeDepth called while not on the event dispatch thread!");
			}
			synchronized (RJOldGUIListener.this) {
				++change_depth_count;
	...
	public void parameterChange (ParameterChangeEvent event) {
		...
		try (
			...
			throw new IllegalStateException ("RJOldGUIListener.parameterChange - Caught GUIEDTException, which should never be thrown", e);
		}
