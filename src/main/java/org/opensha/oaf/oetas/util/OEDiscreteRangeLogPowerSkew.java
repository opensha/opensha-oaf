...
/**
 * A discrete range of parameter values, for operational ETAS -- logarithmic scale with power-law skew.
 ...
 */
public class OEDiscreteRangeLogPowerSkew extends OEDiscreteRange {
	...
	// Points are distributed linearly according to a power law (z - c)^m where
	// z == log(x) and m == range_power.
	...
	private double lin_fcn (double x) {
		return Math.pow ((Math.log(x) - c) * dir, range_power);
	}
	...
	private double inv_lin_fcn (double w) {
		return Math.exp (c + (dir * Math.pow (w, 1.0/range_power)));
	}
	...
	private double dlogx_dw (double x) {
		return Math.pow ((Math.log(x) - c) * dir, 1.0 - range_power) * dir / range_power;
	}
	...
	private void setup (int the_range_size, double the_range_min, double the_range_max, double the_range_skew, double the_range_power) {
		...
		if (!( the_range_size >= 1 )) {
			throw new IllegalArgumentException ("OEDiscreteRangeLogPowerSkew.setup: Range size is non-positive: range_size = " + the_range_size);
		}

		if (!( the_range_min > 0.0 && the_range_max > 0.0 )) {
			throw new IllegalArgumentException ("OEDiscreteRangeLogPowerSkew.setup: Range limits are non-positive: range_min = " + the_range_min + ", range_max = " + the_range_max);
		}
		...
		if (!( (the_range_skew >= CHECK_MIN_HI_RANGE_SKEW && the_range_skew <= CHECK_MAX_HI_RANGE_SKEW)
			...
			throw new IllegalArgumentException ("OEDiscreteRangeLogPowerSkew.setup: Range skew is invalid: range_skew = " + the_range_skew);
		}
		...
		if (!( (the_range_power >= CHECK_MIN_HI_RANGE_POWER && the_range_power <= CHECK_MAX_HI_RANGE_POWER)
			...
			throw new IllegalArgumentException ("OEDiscreteRangeLogPowerSkew.setup: Range power is invalid: range_power = " + the_range_power);
		}
		...
		// Otherwise, we have a multi-point range
			...
			if ((range_power >= 1 && range_skew >= 1) || (range_power < 1 && range_skew < 1)) {
				...
				c = Math.log(range_min) - (Math.log(range_max/range_min) / ( Math.expm1 (Math.log(range_skew) / (range_power - 1.0)) ));
			}
			...
			// Otherwise ...
				...
				c = Math.log(range_max) + (Math.log(range_max/range_min) / ( Math.expm1 (Math.log(range_skew) / (1.0 - range_power)) ));
			}
		}
	...
	public double[] get_bin_array () {
		...
		if (range_size > 1) {
			...
			final double whi = lin_fcn (range_max);
			final double w_delta = (whi - wlo) / ((double)(range_size - 1));
			result[0] = Math.exp ( Math.log (range_min) - (0.5 * w_delta * dlogx_dw (range_min)) );
			result[range_size] = Math.exp ( Math.log (range_max) + (0.5 * w_delta * dlogx_dw (range_max)) );
	...
	public OEDiscreteRangeLogPowerSkew () {}


	// Construct from given parameters.

	public OEDiscreteRangeLogPowerSkew (int the_range_size, double the_range_min, double the_range_max, double the_range_skew, double the_range_power) {
	...
	public String toString() {
		return "RangeLogPowerSkew[range_size=" + range_size
		+ ", range_min=" + range_min
		+ ", range_max=" + range_max
	...
	private static final int MARSHAL_VER_1 = 142001;

	private static final String M_VERSION_NAME = "OEDiscreteRangeLogPowerSkew";

	// Get the type code.

	@Override
	protected int get_marshal_type () {
		return MARSHAL_LOG_POWER_SKEW;
	}
	...
	protected void do_umarshal (MarshalReader reader) {
		...
		default:
			throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_umarshal: Unknown version code: version = " + ver);
		...
		case MARSHAL_HWV_1: {
			...
			try {
				...
				throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_umarshal: Range has invalid parameters", e);
			}
		}
		break;
	...
	@Override
	public OEDiscreteRangeLogPowerSkew unmarshal (MarshalReader reader, String name) {
	...
	public static void marshal_poly (MarshalWriter writer, String name, OEDiscreteRangeLogPowerSkew obj) {
	...
	public static OEDiscreteRangeLogPowerSkew unmarshal_poly (MarshalReader reader, String name) {
		OEDiscreteRangeLogPowerSkew result;
		...
		default:
			throw new MarshalException ("OEDiscreteRangeLogPowerSkew.unmarshal_poly: Unknown class type code: type = " + type);
	...
	protected void do_marshal_friendly (MarshalWriter writer, String name) {
		...
		writer.marshalString (null, MF_KEY_LOG_POWER_SKEW);
	...
	protected void do_umarshal_friendly (MarshalReader reader, int n) {
		...
		try {
			...
			throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_umarshal_friendly: Range has invalid parameters", e);
		}
	...
	protected void do_marshal_xver (MarshalWriter writer, int xver) {
		...
		default:
			throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_marshal_xver: Unknown version code: version = " + xver);
		...
		case XVER_1: {
			...
			writer.marshalString (MF_TAG_KIND, MF_KEY_LOG_POWER_SKEW);
	...
	protected void do_umarshal_xver (MarshalReader reader, int xver) {
		...
		default:
			throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_umarshal_xver: Unknown version code: version = " + xver);
		...
		case XVER_1: {
			...
			try {
				...
				throw new MarshalException ("OEDiscreteRangeLogPowerSkew.do_umarshal_xver: Range has invalid parameters", e);
			}
		}
		break;
