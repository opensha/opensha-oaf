...
// Bug fix for AbstractParameterEditor.
// Author: Michael Barall 10/27/2021.
//
// This is the code of AbstractParameterEditor with a bug fix.
//
// The AbstractParameterEditor constructor calls refreshParamEditor() which in
// turn calls buildWidget(), which is an abstract function implemented in the
// subclass.  As a result, the subclass is called before its constructor
// executes and before its instance variables are initialized, leading to
// subtle (or not-so-subtle) bugs.
//
// To fix this, the call to refreshParamEditor() has been removed from the
// constructor.  Instead, the subclass constructor must call refreshParamEditor()
// after initializing itself.  Generally, the initialization done prior to
// calling refreshParamEditor() should be limited to the subclass setting up its
// own instance variables, because this class will not be fully initialized yet.
//
// Other than the need to call refreshParamEditor() from the subclass constructor,
// this class is a drop-in replacement for AbstractParameterEditor.

public abstract class GUIAbstractParameterEditor<E> extends LabeledBorderPanel implements ParameterEditor<E> {
...
//	public static Color FORE_COLOR = new Color( 80, 80, 140 );
	 ...
	 */
	public GUIAbstractParameterEditor() {
		this(null);
	}
	 ...
	 * @param param
	 */
	public GUIAbstractParameterEditor(Parameter<E> param) {
...
//		setTitleFont(DEFAULT_LABEL_FONT);
		...
		// Code for setParameter inserted here, except the final call to refreshParamEditor
		...
		if (!isParameterSupported(param)) {
			if (param == null)
				...
				throw new IllegalArgumentException(
						"Parameter '"+param.getName()+"' of type '"
						+ClassUtils.getClassNameWithoutPackage(param.getClass())
